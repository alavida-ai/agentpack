...
```
  Author         Validate        Publish         Install+Materialize Discover
 ────────►     ────────►      ────────►       ────────►            ────────►
 knowledge →   structure →    npm package →   npm install →        agent sees
 SKILL.md      checks pass    on registry     materialize          the skill
```
...
This happens through `agentpack materialize` (for published packages) and `agentpack author dev` (for local authoring). You rarely need to think about it, but it explains why agentpack exists as a layer on top of plain npm -- npm handles resolution and install, agentpack handles the last mile of making skills visible to agents.
...
- Root `SKILL.md` is the primary export; `skills/**/SKILL.md` provides named exports from the filesystem
