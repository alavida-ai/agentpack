...
<Step title="Enable the skill for agent discovery">

```bash
agentpack skills enable @acme/brand-copywriting
```

agentpack reads the installed package's skill exports from `node_modules` and materializes them into `.claude/skills/` and `.agents/skills/` where agents can discover them. The enabled-target state is recorded in `.agentpack/install.json`.
...
Shows installed packages, their exports, and which runtimes they are enabled for.
...
Update the package with npm, then re-enable to refresh the materialized skills.
...
| Enable | `agentpack skills enable <package>` | Consumer |
...
See [package.json Reference](/schema-package-json) for all fields including multi-skill packages with `agentpack.root`.
...
For automated install and enable in CI:

```yaml .github/workflows/enable-skills.yml
- name: Install and enable skills
  env:
    NODE_AUTH_TOKEN: ${{ secrets.GITHUB_PACKAGES_TOKEN }}
  run: |
    npm install @acme/brand-copywriting
